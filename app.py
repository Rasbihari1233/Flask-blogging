...
def blog():
    ...
    return render_template('Blog-home2.html', parameters=parameters, posts=posts, prev=prev_page, next=next_page, recent_posts=recent_posts)
...
def post_route(post_slug):
    ...
    recent_posts = Posts.query.order_by().all()[0:3]
    # post = Posts.query.filter_by().first(slug=post_slug) [0 : parameters['no_of_posts']]
    return render_template('post2.html', parameters=parameters, post=post, recent_posts=recent_posts)
...
@app.route("/python")
def python():
    return render_template('python/python.html', parameters=parameters)
